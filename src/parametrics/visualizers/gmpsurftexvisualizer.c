...
/*! \file gmpsurftexvisualizer.c
 *
 *  Implementation of the PSurfTexVisualizer template class.
 */
...
namespace GMlib {
  ...
  template <typename T>
  PSurfTexVisualizer<T>::PSurfTexVisualizer() : _vbo(), _ibo(), _no_vertices(0) {
  ...
  template <typename T>
  PSurfTexVisualizer<T>::~PSurfTexVisualizer() {
  ...
  template <typename T>
  inline
  void PSurfTexVisualizer<T>::display() {
    ...
    prog.setUniform( "u_tex", _tex, (GLenum)GL_TEXTURE1, 1 );

    GLuint vert_loc = prog.getAttributeLocation( "in_vertex" );
    GLuint tex_loc = prog.getAttributeLocation( "in_tex" );

    glEnablei( GL_BLEND, OGL::getRenderBuffer() );
    ...
    glDisablei( GL_BLEND, OGL::getRenderBuffer() );
  }

  template <typename T>
  std::string PSurfTexVisualizer<T>::getIdentity() const {

    return "PSurf Tex Visualizer";
  }

  template <typename T>
  inline
  void PSurfTexVisualizer<T>::replot(
  ...
  template <typename T>
  inline
  void PSurfTexVisualizer<T>::select() {
