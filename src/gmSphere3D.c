...
namespace GMlib {
  ...
  void Sphere3D::makeSample(int m1, int m2) {
    ...
    _m1 = m1-1; _m2 = m2+1;
    _pt[0] = Arrow<float,3>(_pos+Point3D<float>(0,0,r), Vector3D<float>(0,0,1));
    _pt[1] = Arrow<float,3>(_pos+Point3D<float>(0,0,-r),Vector3D<float>(0,0,-1));
    du = M_PI/m1;
    dv = M_2PI/m2;
    _p.setSize(_m1);
    u = M_PI_2-du;
    for(int i=0; i<_m1; i++,u-=du)
    {
      ...
      for(int j=0; j<_m2; j++, v+=dv)
      {
        sv = (float)sin(v);
        cv = (float)cos(v);
        _p[i][j] = Arrow<float,3>(_pos+Point3D<float>(ru*cv,ru*sv,r*su),Vector3D<float>(cu*cv,cu*sv,su));
      }
    }
  }
